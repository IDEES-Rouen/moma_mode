...
global //schedules: ([world] + SpatObj + shuffle(Aedes) + Stat)
{
	...
	int sim_nb_daysInYear <- 1; // modifs renaud --> 11/07/2015
	...
	{
		//write string(cpt_day) + " -> nb of aedes: " + length(Aedes);
	...
	action createSpatObj {
		...
		{
			/*if (shape.area > 1000) {
				list<geometry> rects <- to_squares(shape, 100.0);
	...
	reflex too_much_aedes when: length(Aedes) > 30000 {
		...
		global_error <-cycle = 0  ? #max_float : length(Aedes)/cycle * 1000;
		ask Aedes {do die;}
		fin <- true;
		do save_results_fin;
		fin_cycle <- cycle + 100;
	...
	action save_results {
		string results <- "NB_ADULT_FEMALE: "+  NB_ADULT_FEMALE + " coeff_larve: " + coeff_larve + " coeff_oeuf: "+ coeff_oeuf + " coeff_nymphe:" + coeff_nymphe+ " u_lambda:" + u_lambda
			+ " a_lambda: " + a_lambda + " b_lambda:" + b_lambda + " dist_gitesout: " + dist_gitesout + " AVG_EGG_BSSURFACE: " + AVG_EGG_BSSURFACE + " global_error:" + global_error + " cycle: " + cycle ;
...
//			}
		...
		action rest { // modifs renaud
			...
			if (dist_piege < dist_min and !capture_try and mySpatObj.contient_piege = true and  flip(BGtrap_rate) and
				(((mySpatObj.iNbBsOutWithWater + mySpatObj.iNbBsInWithWater) = 0 ) or flip(length(mySpatObj.mes_pieges) / (mySpatObj.iNbBsOutWithWater + mySpatObj.iNbBsInWithWater)) )
			) { // Le moustique est capturé
...
//					//c'est pour éviter de la division par zéro?
	...
	species SpatObj
	{
		...
		{		 
			...
			geom_limit <- union(my_neighbors where (shape.area > 0));
...
//			}	
			...
			write "iNbNewFemaleA: " + iNbNewFemaleA; 
...
//		}	
		...
		display chart refresh: every(10)
		{
			...
			{
				data "Nb moustiques" value: length(Aedes) color: #blue;
				data "Nb moustiques captures" value: sum (Piege collect each.n_captured) color: #magenta;
				data "Errors" value: global_error color: #red;
			} 
...
experiment 'calibration' type: batch repeat: 1 keep_seed: true until: fin_sim //nb_days = sim_days_duration or length(Aedes) = 0 
{
	...
	method genetic pop_dim: 10 crossover_prob: 0.7 mutation_prob: 0.1 nb_prelim_gen: 1 max_gen: 100  minimize: global_error ;
