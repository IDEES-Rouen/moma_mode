...
global //schedules: ([world] + SpatObj + shuffle(Aedes) + Stat)
{
	...
	int first_day <- 192;
	int last_day <- 225;
	list<int> day_obs <- [5,8,12,15,19,21,26,29,33,36];
	float total_local_error;
	float global_error;
	file grid_ref <- file('../num_data/meteo/air_temp/ta_day_A2015192.tif') ; // Premier jour à changer
	
	int u_lambda <- 9;
	int a_lambda <- 12;
	int b_lambda <- 7;
	int dist_gitesout <- 80;
	float BGtrap_rate <- 0.25;
	float dist_min <- 3.0;
	 ...
	 ********************************/
	 //spatial data
	 file Piege_shape <- file("../shp/pieges/pieges.shp");
	 file SpatObj_shape <- file ("../shp/wp/watpuranawat.shp");// parameter: "Shapefile for the study zone" category: "GIS" ;
	 file SpatObjType_file <- csv_file("../shp/wp/landuse_watpuranawat.csv");// parameter: "land use classes for the shapefile" category: "GIS";
	 file grid_pop <- file('../num_data/pop/wp_popest_30m.tif');
	 file grid_men <- file('../num_data/pop/wp_menest_30m.tif');
	 geometry shape <- envelope(SpatObj_shape);

	 file grid_gites_in <- file('../num_data/gites/wp_giteest_in_30m.tif');
	 file grid_gites_out <- file('../num_data/gites/wp_giteest_out_30m.tif');	 
	 
	//-------------- meteo data-------------------------	
	file temperatureFile <- csv_file("../num_data/meteo/wp_2015.csv", "	") ;//parameter: "temperature file" category: "Weather";
	file sunFile <- csv_file("../num_data/meteo/sun_wp_2015.csv", "	");// parameter: "sunset-sunrise file" category: "Weather";
	
	matrix temperature_matrix <- matrix(temperatureFile);
	matrix<float> na_matrix <- 0.0 as_matrix {24,length(temperature_matrix column_at 0)-1}; //Matrice pour remplir les données NA des températures satellites
	...
	int sim_nb_daysInYear <- 0; // modifs renaud --> 11/07/2015
	int iSimRealDay update: nb_days + sim_nb_daysInYear;
	int sim_days_duration <- last_day - first_day + 1; // How many days of simulation?
...
/*		mn_sunrise <- int((list_sunrise at (nb_days) - int(list_sunrise at (nb_days)))*100);
	...
	reflex updateRainfalls when: bNewDay {
		...
		fGlobalTodayAirTempC <- (list_daily_max_temp[nb_days] + list_daily_min_temp[nb_days])/2;
		fGlobalTodayRainfall <- list_daily_rain[nb_days];
		write "" + nb_days + " --> " + fGlobalTodayRainfall; 
	}
	...
	action createSpatObj {
		...
		{
			if (shape.area > 500) {
				list<geometry> rects <- to_squares(shape, 50.0);
				loop rect over: rects {
					create SpatObj with:[shape::rect, strClass::strClass];
				}
				do die;
			}
		}
	...
	action loadClassLanduse
	{
		if(bAvgDenseBSMode) //si oui, create land use class avec la valeur moyenne du bs in and out
		{
			...
			ask Class_Landuse {
				//write('fDenBsOutNoWater = ' + self.fDenBsOutNoWater);
			}
		}
		else
		{
	...
	rgb adjustObjColor(rgb myCol)
  	{
  		rgb newCol <- rgb("black");
  		switch (myCol)
  		{
  			match rgb("magenta") {newCol <- rgb(255,204,204);}
  			match rgb("yellow") {newCol <- rgb(253,234,218);}
  			match rgb("red") {newCol <- rgb(192,80,77);}
  			match rgb("brown") {newCol <- rgb(204,102,0);}
  			match rgb("green") {newCol <- rgb(155,187,89);}
  			match rgb("grey") {newCol <- rgb(100,100,100);}
  			match rgb("blue") {newCol <- rgb(85,142,220);}
  			match rgb("black") {newCol <- rgb(220,220,220);}
  		}
  		return newCol;
  	}
  	...
  	action create_oeufs_larves_nymphes(list<SpatObj> bornObjs) {
  		ask bornObjs {
				...
				loop times:int(coeff_oeuf)   {
					if (inside) {
						int ind <- rnd(length(fListStkEtoLIn) - 1);
						fListStkEtoLIn[ind]<- fListStkEtoLIn[ind] + 1;
					} else {
						int ind <- rnd(length(fListStkEtoLOut) - 1);
						fListStkEtoLOut[ind]<- fListStkEtoLOut[ind] + 1;
					}
					
				}
				loop times:int(coeff_larve)   {
					if (inside) {
						int ind <- rnd(length(fListStkLtoPIn) - 1);
						fListStkLtoPIn[ind]<- fListStkLtoPIn[ind] + 1;
					} else {
						int ind <- rnd(length(fListStkLtoPOut) - 1);
						fListStkLtoPOut[ind]<- fListStkLtoPOut[ind] + 1;
					}
					
				}
				loop times:int(coeff_nymphe)   {
					if (inside) {
						int ind <- rnd(length(fListStkPtoAIn) - 1);
						fListStkPtoAIn[ind]<- fListStkPtoAIn[ind] + 1;
					} else {
						int ind <- rnd(length(fListStkPtoAOut) - 1);
						fListStkPtoAOut[ind]<- fListStkPtoAOut[ind] + 1;
					}
	 ...
	 ***************************************************************/
	action createHourAirTempMatrix { // modifs renaud
		
		loop i from: 1 to: length(temperature_matrix column_at 0)-1 { //a line  = a day
			loop j from: 0 to: 23 { // column for each hour
				na_matrix[j,i-1] <- calHourlyTempK(j,i, float(temperature_matrix[1,i]), float(temperature_matrix[2,i]), float(temperature_matrix[2,i+1]));
			}
		}
		...
		loop i from: first_day to: last_day-1 {	
			// Définition de la partie du nom du fichier associée au numéro du jour j et de j+1  
			int_day <- i; 
			int_dayplus1 <- i + 1;

			if (int_day < 10) {str_day <- '00' + string(int_day);}
			else if (10 <= int_day and int_day < 100) {str_day <- '0' + string(int_day);} 
			else {str_day <- string(int_day);}

			if (int_dayplus1 < 10) {str_dayplus1 <- '00' + string(int_dayplus1);}
			else if (10 <= int_dayplus1 and int_dayplus1 < 100) {str_dayplus1 <- '0' + string(int_dayplus1);}
			else {str_dayplus1 <- string(int_dayplus1);}

			// Chargement des rasters de température day de j, night de j, day de j+1
			matrix ta_night <- matrix(csv_file('../num_data/meteo/air_temp/ta_night_A' + string(year) + str_day + '.csv', " ")); // Chargement du fichier night du jour i
			matrix ta_day <- matrix(csv_file('../num_data/meteo/air_temp/ta_day_A' + string(year) + str_day + '.csv', "")); // Chargement du fichier day du jour i
			matrix ta_nightplus1 <- matrix(csv_file('../num_data/meteo/air_temp/ta_night_A' + string(year) + str_dayplus1 + '.csv', "")); // Chargement du fichier night du jour i
			

			// Calcul des températures de chaque heure pour tous les jours de la période d'étude (définie au début dans les variables first_day et last_day) 
			ask cell {
				bool is_na <- false;
				float tmax <- float(ta_day[grid_x,grid_y+6]);
				float tmin <- float(ta_night[grid_x,grid_y+6]);
				float tminplus1 <- float(ta_nightplus1[grid_x,grid_y+6]);
				
				if (tmax = -9999.0 and tmin = -9999.0 and tminplus1 = -9999.0) {
					is_na <- true;
				}
				if (tmax = -9999.0){tmax <- temperature_matrix[1,i];}
				if (tmin = -9999.0){tmin <- temperature_matrix[2,i];}
				if (tminplus1 = -9999.0){tminplus1 <- temperature_matrix[2,i+1];}				
				
				loop j from: 0 to: 23 { // column for each hour
					if (is_na) {
						self.ta[j,i-first_day] <- na_matrix[j,i-first_day];
					} else {
						self.ta[j,i-first_day] <- world.calHourlyTempK(j, i, tmax, tmin, tminplus1);
					}
				}
			}
		}
	...
	// Distribution des ménages et des habitants dans les bâtiments
	action distribuer_popmen { // On donne à chaque bâtiment vide un ménage. S'il n'y a plus de bâtiment vide, on choisit le bâtiment le plus proche
		
		int tot_men <- 0;
		bool test <- true;
		
		ask cell_men {
			if self.grid_value < 0 {self.grid_value <- 0;} // Certaines cellules en "NoData" ont une valeur inférieure à 0 ?
			self.grid_value <- round(self.grid_value);
			tot_men <- tot_men + self.grid_value;
			loop while: (self.grid_value > 0) { // Tant qu'il y a des ménages dans la cellule
				// Listage des bâtiments libres (sans ménage)
				list<SpatObj> bat_libre <- SpatObj where (each.strClass = "builtup" and each.nmen = 0);
				
				if length(bat_libre) = 0 { // Si la liste des bâtiments libre est supérieure à 0
					// Identification du bâtiment auquel le ménage sera attribué
					list<SpatObj> bat_libre <- SpatObj where (each.strClass = "builtup" and each.nmen >= 1);
					SpatObj batiment <- bat_libre closest_to self;

					// Déplacement du ménage dans le bâtiment attribué
					batiment.nmen <- batiment.nmen + 1;
					self.grid_value <- self.grid_value - 1;
					
					// Ajout des gîtes intérieurs dans le bâtiment attribué au ménage
					batiment.iNbBsInNoWater <- int(poisson(b_lambda)) + batiment.iNbBsInNoWater;

					// Ajout des gîtes extérieurs dans les objets situés autour du bâtiment attribué au ménage
					int gitesOut <- int(poisson(a_lambda)); // Nombre de gites out total produits par le ménage
					
					list<SpatObj> SpatObj_voisins;
					ask batiment {
						SpatObj_voisins <- SpatObj at_distance dist_gitesout; // Liste des objets se situant à moins de X m autour du bâtiment habité par le ménage
					}
					loop i from: 0 to: length(SpatObj_voisins) - 1 { // Attribution des gites out aux objets aux alentours
						ask SpatObj_voisins[i] {
							self.iNbBsOutNoWater <- int(gitesOut/length(SpatObj_voisins)) + self.iNbBsOutNoWater;	
						}
					}
				}
				else { // Si la liste des bâtiments libre est = à 0
					// Identification du bâtiment attribué au ménage
					SpatObj batiment <- bat_libre closest_to self;				

					// Le ménage récupère ses gites extérieur d'origine et les disperse autour de son nouveau lieu d'habtation
					list<cell_gites_out> cell_gites_out_voisins <- cell_gites_out at_distance dist_gitesout; // Liste des cell_gites_out situées à moins d'un mètre de la cell_men en cours 
					// Déplacement du ménage dans le bâtiment attribué
					batiment.nmen <- batiment.nmen + 1;
					self.grid_value <- self.grid_value - 1;
					...
					list<SpatObj> SpatObj_voisins;
					ask batiment {
						SpatObj_voisins <- SpatObj at_distance 30; // Liste des objets se situant à moins de X m autour du bâtiment habité par le ménage
					}
					loop i from: 0 to: length(SpatObj_voisins) - 1 {					
						ask SpatObj_voisins[i] {
							self.iNbBsOutNoWater <- int(gitesOut/length(SpatObj_voisins)) + self.iNbBsOutNoWater;
						} 
					}
		...
		// Distribution des habitants (exactement le même nombre d'habitants par ménage)
		int tot_pop <- 0;
		ask cell_pop {
			if self.grid_value < 0 {self.grid_value <- 0;}
			tot_pop <- tot_pop + round(self.grid_value);
		}
		int nhab_moy <- round(tot_pop / tot_men); 
		list<SpatObj> bat_habite <- SpatObj where (each.strClass = "builtup" and each.nmen >= 1);
		ask bat_habite {
			self.nhab <- self.nmen * nhab_moy;
			self.fInitPop <- self.nhab; // modifs renaud
			self.fHourDensPop <- self.fInitPop/self.shape.area;
		}
		//write("Nombre d'habitants total = " + tot_pop);
		//write("Nombre de ménages total = " + tot_men);
	}
	
	action createPieges {
		create Piege from: Piege_shape with: [situation::string(read("SITUATION")), nb_aedes_obs::list<int>([int(read("AEDES_13_0")),int(read("AEDES_16_0")),int(read("AEDES_20_0")),int(read("AEDES_23_0")),int(read("AEDES_27_0")),int(read("AEDES_29_0")),int(read("AEDES_03_0")),int(read("AEDES_06_0")),int(read("AEDES_10_0")),int(read("AEDES_13_2"))])] {
			n_captured <- 0; // Nombre de moustiques capturés initial
			if (situation = 'in') {
				SpatObj mon_batiment <- SpatObj where (each.strClass = "builtup") closest_to self;
				mon_batiment.contient_piege <- true; 
				mon_batiment.iNbBsInNoWater <- mon_batiment.iNbBsInNoWater + 1; // On ajout un gite intérieur au bâtiment du piège
				add self to: mon_batiment.mes_pieges;
				//write('piege n°'+ self + 'in créé dans ' + mon_batiment);
			} else {
				SpatObj mon_SpatObj <- SpatObj where (each.strClass != "builtup") closest_to self;
				mon_SpatObj.contient_piege <- true; 
				mon_SpatObj.iNbBsOutNoWater <- mon_SpatObj.iNbBsOutNoWater + 1; // On ajout un gite extérieur au bâtiment du piège
				add self to: mon_SpatObj.mes_pieges;
				color <- rgb(192,80,77);
				//write('piege ext créé dans ' + mon_SpatObj);
			}
		}
		ask SpatObj {
			nb_pieges <- length(mes_pieges);
			if (nb_pieges > 1) {
				Piege piegePrinc <- first(mes_pieges);
				ask (mes_pieges - piegePrinc) {
					loop i from: 0 to: length(nb_aedes_obs) - 1 {
						piegePrinc.nb_aedes_obs[i] <- piegePrinc.nb_aedes_obs[i] + nb_aedes_obs[i];
					}
					do die;
				}
			}
	...
	reflex too_much_aedes when: length(Aedes) > 30000 {
		
		total_local_error <-cycle = 0  ? #max_float : length(Aedes)/cycle * 1000;
		ask Aedes {do die;}
		fin <- true;
		do save_results_fin;
		fin_cycle <- cycle + 100;
	...
	action save_results {
		string results <- "NB_ADULT_FEMALE: "+  NB_ADULT_FEMALE + " coeff_larve: " + coeff_larve + " coeff_oeuf: "+ coeff_oeuf + " coeff_nymphe:" + coeff_nymphe+ " u_lambda:" + u_lambda
			+ " a_lambda: " + a_lambda + " b_lambda:" + b_lambda + " dist_gitesout: " + dist_gitesout + " AVG_EGG_BSSURFACE: " + AVG_EGG_BSSURFACE + " global_error:" + total_local_error + " cycle: " + cycle ;
		save results to: "results.txt";
		string id <- "" + cycle + "_"+ NB_ADULT_FEMALE+ "_"+ coeff_larve+ "_"+ coeff_oeuf+ "_"+ coeff_nymphe+ "_"+ u_lambda+ "_"+ a_lambda+ "_"+ b_lambda+ "_"+ dist_gitesout+ "_"+ AVG_EGG_BSSURFACE;
		save Piege type:"shp" to:"results/" + id + "_pieges.shp" with: [error::"ERROR"];
		write results;
	}
	
	action save_results_fin {
		string results <- "NB_ADULT_FEMALE: "+  NB_ADULT_FEMALE + " coeff_larve: " + coeff_larve + " coeff_oeuf: "+ coeff_oeuf + " coeff_nymphe:" + coeff_nymphe+ " u_lambda:" + u_lambda
			+ " a_lambda: " + a_lambda + " b_lambda:" + b_lambda + " dist_gitesout: " + dist_gitesout + " AVG_EGG_BSSURFACE: " + AVG_EGG_BSSURFACE + " global_error:" + global_error + " total_local_error:" + total_local_error + " cycle: " + cycle ;
		save results to: "results_fin.txt";
		write results;
	}
	
	reflex computing_error when: bNewDay and ((nb_days + 1) in day_obs) {
		int index <- day_obs index_of(nb_days + 1);
		global_error <- global_error + abs(sum(Piege collect each.n_captured) - sum (Piege collect each.nb_aedes_obs [index] ));
		
		ask Piege {
			error  <- float(abs(nb_aedes_obs [index] - n_captured));
			total_local_error <- total_local_error + error;
			n_captured <- 0;
		}
		//do save_results;
		fin <- (nb_days + 1)= last(day_obs) ;
		if (fin) {
			fin_cycle <- cycle + 100;
			ask Aedes {do die;}
			do save_results_fin;
	 ...
	 **************************************************************************/
	float calHourlyTempK(float calTime, int nb_days, float tmax, float tmin, float tmin_jplus1) { //modifs renaud
		//initial the variables for air temperature
		float tdl <- float(sun_matrix[2,nb_days]) - float(sun_matrix[1,nb_days]); //tdl = sunset - sunrise
		float ttmax <- 15.0 - 13.0; // Time at tmax - solar noon
		
		return 273.15 + tmin + (tmax - tmin) * (sin(#pi * (calTime + tdl / 2 - 12) / (tdl + 2 * ttmax)) + 1) / 2; // Zhu 2013 modifiée pour que le min est ne soit pas inférieur au min obs
	} 
}
...
grid cell file: grid_ref {
	matrix<float> ta <- 0.0 as_matrix {24, last_day - first_day}; // + 1 enlevé à la fin
}
/////////////////////////////////////////////////////////
// 				GRILLES DE POPULATIONS (tif des pop et des gites ont la même résolution et doivent être mis en acs
/////////////////////////////////////////////////////////

grid cell_pop file: grid_pop {

}

grid cell_men file: grid_men {
...
grid cell_gites_in file: grid_gites_in {

}

grid cell_gites_out file: grid_gites_out {
...
//			do layEgg; 
		 ...
		 * "hourlyPhysioDevGono" fluctuate between [0, 1]
		 /
		 ***********************************************************/
		reflex updateGonoPhysioDev when: bGonoBegan and bNewHour and !bPhysioDevDone // modifs renaud
		{
			float t <- bVarComputeExecTime ? machine_time : 0; 
 ...
 */
				cell closest_cell <- cell closest_to self;
				float actual_temp <- closest_cell.ta[int(curTime/298),iSimRealDay];
				
				if(actual_temp != 0) { //in case of divide by 0
					hourlyPhysioDevGono <- RO25_GONO * (actual_temp * exp(DHA_GONO / R * (1 / 298 - 1 / actual_temp))) / (1 + exp(DHH_GONO / R * (1 / THALF_GONO - 1 / actual_temp)));
				}
...
//			}
		 ...
		 *
		 *******************/
		action rest { // modifs renaud
			cpt_action <- cpt_action + 1;
			float dist_piege <- self distance_to (Piege closest_to self);
			//if (dist_piege < dist_min and !capture_try and mySpatObj.contient_piege = true and flip(length(mySpatObj.mes_pieges) / (mySpatObj.iNbBsOutWithWater + mySpatObj.iNbBsInWithWater)) and flip(BGtrap_rate)) { // Le moustique est capturé
			if (dist_piege < dist_min and !capture_try and mySpatObj.contient_piege = true and  flip(BGtrap_rate) and
				(((mySpatObj.iNbBsOutWithWater + mySpatObj.iNbBsInWithWater) = 0 ) or flip(mySpatObj.nb_pieges / (mySpatObj.iNbBsOutWithWater + mySpatObj.iNbBsInWithWater + mySpatObj.nb_pieges)) )
			) {
				Piege piege_capturant <- mySpatObj.mes_pieges closest_to self;
				piege_capturant.n_captured <-piege_capturant.n_captured + 1; 
				//write('Moustique capturé dans le piège ' + piege_capturant);
				do killMe("capture"); // Le moustique meurt
			} else {
		...
		action wakeUp
		{
			if(strState != "gono") or (strState ="gono" and bNeedBloodGono) or (strState = "gono" and bPhysioDevDone) 
			{
				...
				capture_try <- false; // modif renaud
		 ...
		 *******************/			
		action layEgg { // modifs renaud
			//write('Je veux pondre des oeufs');
			float dist_piege <- self distance_to (Piege closest_to self);
			if (dist_piege < dist_min and !capture_try and mySpatObj.contient_piege = true and flip((mySpatObj.nb_pieges) / (mySpatObj.iNbBsOutWithWater + mySpatObj.iNbBsInWithWater + mySpatObj.nb_pieges)) and flip(BGtrap_rate)) { // Le moustique est capturé
				Piege piege_capturant <- mySpatObj.mes_pieges closest_to self;
				piege_capturant.n_captured <-piege_capturant.n_captured + 1;
				//write('Moustique capturé dans le piège ' + piege_capturant);
				do killMe("capture"); // Le moustique meurt
			} else { // Le moustique n'est pas capturé et continue son activité normale
				int eggsLaid <- 0;
				float fBloodUsedLayEgg <- 0.0;
				float fSpeed <- (rnd((MAX_SPEED_LAYEGG - MIN_SPEED_LAYEGG)*10000)/10000 + MIN_SPEED_LAYEGG);
			 	bsToOvipo <- mySpatObj;
				capture_try <- true; // modif renaud
		...
		{	
			...
			if(!empty(list_detectedObjs))
			{
				...
				loop i from: 0 to: length(list_detectedObjs) - 1 // for all detected objs
				{
					...
					if (interZone = nil or empty(interZone.points) or  interZone.perimeter = 0) {
						interZone <- list_detectedObjs[i].shape;
...
//					//c'est pour éviter de la division par zéro?
		...
		action checkPhysioDevDone
		{
			...
			if(actual_temp >= MIN_TEMP_LAYEGG + 273.15)//Â°K
			{ 	
		...
		*************************************************/
		...
		aspect default
		{
			...
			if(bTrackAedes) 
			{
	 			...
	 			draw string(strTarget) size: 20 color: rgb('black'); 
	 		}			
		}
	}
	...
	species SpatObj
	{
		...
		bool contient_piege <- false;
		list<Piege> mes_pieges <- nil;
		int nb_pieges <- 0;
		
		///// for bBsVarMode only /////////
		int iNbBsInNoWater <- 0;
		int iNbBsOutNoWater <- poisson(u_lambda);
		////////////////////////////
		...
		{		 
			...
			if(bAvgDenseBSMode) { // modifs renaud
				// Les ménages prennent leurs gites_in dans leur nouvel objet spatial mais laisse leurs gites_out
				//Les gîtes intérieurs initiaux dépendent principalement des ménages, ils sont donc init avec eux
								
				// Les gîtes exterieurs sont initialisés ici (à changer peut-être puisqu'ils dépendent des ménages autours...)
				ask cell_gites_out {
					bool is_intersecting <- self intersects myself; //
					if is_intersecting {
//						float area_inter <- (self inter myself).area;
//						myself.iNbBsOutNoWater <- myself.iNbBsOutNoWater + round(self.grid_value * area_inter / 900);
//						write('Nb gites out = ' + myself.iNbBsOutNoWater);
					}
				}
				
				// Calcul de la surface d'intersection entre une cellule et un objet
...
//				write('Gites our = ' + spcClass.fDenBsOutNoWater + ' | Gites in = ' + spcClass.fDenBsInNoWater); 
			}
			else
			{
				do calNbBsNoWater;
...
//				else if (iNbBsOutWithWater < iNbBsOutNoWaterMin){iNbBsOutWithWater <- iNbBsOutNoWaterMin;}
		...
		action calNbBsNoWater
		{
			...
			//write('fDenBsInNoWaterMax = ' + spcClass.fDenBsInNoWaterMax); 
...
//	     	//---------- pupae ----------------------
		...
		* using water temperature equation to calculate max temperature and min temperature of water in container
		...
		{
			cell closest_cell <- cell closest_to self; // modifs renaud
			list<float> temps_day <- closest_cell.ta column_at nb_days;
			float daily_temp_min <- min(temps_day) - 273.15; 
			float daily_temp_max <- max(temps_day) - 273.15;
...
//			fDailyWaterMinTemp <- 5.02 + (0.81* list_daily_min_temp[nb_days]) + (0.001* ((list_daily_max_temp[nb_days])^2)) - (1.36*fSunExpoOut);
			fDailyWaterMaxTemp <- 15.03 + (0.27 * daily_temp_min) + (0.01 * ((daily_temp_max)^2)) + (7.69*fSunExpoOut); // modif renaud
			fDailyWaterMinTemp <- 5.02 + (0.81* daily_temp_min) + (0.001* ((daily_temp_max)^2)) - (1.36*fSunExpoOut);
		 ...
		 ************************************************************************/
		action calDailySpatObjAirTempC // modifs renaud
		{
			cell closest_cell <- cell closest_to self;
			list<float> temps_day <- closest_cell.ta column_at nb_days;
			float daily_temp_min <- min(temps_day); 
			float daily_temp_max <- max(temps_day);
...
//			iSpatObjAirMaxTempC <- int(list_daily_max_temp[nb_days]  * spcClass.fTempVarRate);
			iSpatObjAirMinTempC <- int(daily_temp_min - 273.15);
			iSpatObjAirMaxTempC <- int(daily_temp_max - 273.15);
		...
		aspect bloodAtt
		{
			...
			draw shape empty: false color:tmpColor border: #black; //spcClass.color; // depth: fHeight;
		...
		aspect stock
		{
			...
			draw string(int(totPopAquatic))  size: 3 color: rgb('white') ; 
		}
		...
		aspect showDispersal
		{
			...
			if(bDisprsShpNeed)
			{
				draw myMaxDisp.shape empty: false color: myMaxDisp.color  border: #black; 
			}
...
//		}	
		...
		{
			species SpatObj  refresh: false; // transparency: 0.5;
			species Aedes aspect: default transparency: 0.5; //female_fly;
			species Piege aspect: default;
		}	
		...
		display chart 
		{
			...
			chart "Nb aedes" type: series 
			{
				data "Nb moustiques" value: length(Aedes) color: rgb("magenta");
			} 
		}
...
experiment 'calibration' type: batch repeat: 1 keep_seed: true until: fin_sim //nb_days = sim_days_duration or length(Aedes) = 0 
{
	parameter "NB_ADULT_FEMALE" var: NB_ADULT_FEMALE min: 100 max: 1000 step: 10;
	parameter "coeff_larve" var: coeff_larve min: 0.0 max: 20.0 step: 1.0;
	parameter "coeff_oeuf" var: coeff_oeuf min: 0.0 max: 20.0 step: 1.0;
	parameter "coeff_nymphe" var: coeff_nymphe min: 0.0 max: 20.0 step: 1.0;
	...
	method genetic pop_dim: 10 crossover_prob: 0.7 mutation_prob: 0.1 nb_prelim_gen: 1 max_gen: 100  minimize: total_local_error ;
