...
global //schedules: ([world] + SpatObj + shuffle(Aedes) + Stat)
{
	...
	int first_day <- 1;
	int last_day <- 20;
	file grid_ref <- file('../num_data/meteo/air_temp/ta_day_A2010001.tif') ; // Premier jour à changer
	
	int u_lambda <- 6; // Paramètres lambda pour tirage poisson (gites présents de base dans un objet)
	int a_lambda <- 9; // Paramètres lambda pour tirage poisson (gites extérieurs créés par un ménage dans leur propre objet et dans les objets environnants)
	int b_lambda <- 7; // Paramètres lambda pour tirage poisson (gites intérieurs créés par un ménage dans leur propre objet)
	int dist_gitesout <- 30; // Distance max à laquelle un ménage peut créer un gite extérieur dans un objet
	...
	//-------------- meteo data-------------------------	
	file temperatureFile <- csv_file("../num_data/meteo/bang_na_meteo2010.csv", ";") parameter: "temperature file" category: "Weather";
	...
	action createHourAirTempMatrix { // modifs renaud
		...
		loop i from: first_day to: last_day-1 {	
			...
			ask cell {
				...
				loop j from: 0 to: 23 { // column for each hour
					if (is_na) {
						self.ta[j,i-first_day] <- na_matrix[j,i-1];
//						if(i = 9 and j = 8 or j = 15){  // check if temp is ok
//							//write('NA --> ' + self.ta[j,i-first_day]);
//						}
					} else {
				...
				if length(bat_libre) = 0 { // Si la liste des bâtiments libre est supérieure à 0
					...
					loop i from: 0 to: length(SpatObj_voisins) - 1 { // Attribution des gites out aux objets aux alentours
						ask SpatObj_voisins[i] {
							...
							write('Gites out ajoutés');
							write('Nombre de gites out = ' + batiment.iNbBsOutNoWater);
						}
					}
				}
					...
					if(test){
						write("Nombre de voisins = " + length(cell_gites_out_voisins));
						write(cell_gites_out_voisins);
						test <- false;
					}
					// Déplacement du ménage dans le bâtiment attribué
					batiment.nmen <- batiment.nmen + 1;
					...
					// Ajout des gîtes intérieurs dans le bâtiment attribué au ménage
					batiment.iNbBsInNoWater <- int(poisson(b_lambda)) + batiment.iNbBsInNoWater;
					write('Nombre de gites int' + batiment.iNbBsInNoWater);
...
grid cell file: grid_ref {
	matrix<float> ta <- 0.0 as_matrix {24,last_day-first_day+1};
}
/////////////////////////////////////////////////////////
...
//				write('Gites our = ' + spcClass.fDenBsOutNoWater + ' | Gites in = ' + spcClass.fDenBsInNoWater); 
			...
			fInitPop <- self.nhab; // modifs renaud
...
//			iSpatObjAirMaxTempC <- int(list_daily_max_temp[nb_days]  * spcClass.fTempVarRate);
			iSpatObjAirMinTempC <- int(daily_temp_min);
			iSpatObjAirMaxTempC <- int(daily_temp_max);
